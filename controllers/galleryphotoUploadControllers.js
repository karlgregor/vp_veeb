...
const galleryphotoUploadPagePost = async (req, res) => {
    ...
    try {
        ...
        await sharp(req.file.destination + fileName).composite([{input: "./public/images/test.png", top: 0, left: 0}]).resize(800, 600).jpeg({quality: 80}).toFile("./public/gallery/normal/" + fileName);
        await sharp(req.file.destination + fileName).composite([{input: "./public/images/test.png", top: 0, left: 0}]).resize(100, 100).jpeg({quality: 80}).toFile("./public/gallery/thumbs/" + fileName);
